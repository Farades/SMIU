...
//        }
        ...
        ModbusMasterParams masterParams = new ModbusMasterParams("COM5", 38400, 8, "none", 1, "rtu", false, 100);
        ModbusMaster mbMaster = new ModbusMaster("modbus_in", masterParams);

        ModbusSlaveParams slaveParams = new ModbusSlaveParams(1, ModbusFunction.READ_HOLDING_REGS_3, RegType.INT16DIV100, 1802, 1, 50);
        mbMaster.addSlave(new ModbusSlave("slave1_1", slaveParams));

        HashMap<String, Binding> dirisBindings = new HashMap<String, Binding>();
        dirisBindings.put("Ua", new Binding("modbus_in", "slave1_1", 1802));
        try {
            Device diris = new Device("Diris A40", dirisBindings);
        } catch (InitParamBindingsException ex) {
            ex.printStackTrace();
            System.exit(-1);
        }
